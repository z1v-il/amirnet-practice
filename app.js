...
async function generateAISentence() {
    ...
    const promptText = `Expert test writer for Israeli Amirnet English exam. Generate 1 "Sentence Completion" question (Upper-B2/C1). RULE 1: Academic sentence (15-25 words) testing the word "${targetWord}". RULE 2: CRITICAL - The 3 incorrect options MUST have completely different meanings from the correct word. Do NOT use near-synonyms. RULE 3: CRITICAL - Randomize the academic subject completely (e.g., archaeology, marine biology, linguistics, astrophysics, economics). DO NOT use generic topics like 'global challenges'. RULE 4: Keep HEBREW explanation under 12 words. Format EXACTLY as raw JSON: { "sentence": "...", "options": [{"word": "opt1", "translation": "Hebrew"}, ...], "correctWord": "${targetWord}", "explanation": "Short Hebrew explanation" }`;
...
async function generateRestatement() {
    ...
    const promptText = `Expert Amirnet test writer. Generate 1 "Restatement" question (Upper-B2/C1). RULES: 1. Academic original sentence (18-25 words). CRITICAL: Randomize the sentence structure! Mix Conditionals, Cause/Effect, or Relative clauses. 2. CRITICAL: Randomize the academic subject completely (e.g., ancient history, botany, sociology, quantum physics). Do NOT repeat generic AI topics. 3. Correct restatement MUST alter syntactic structure. 4. Options roughly same length. 5. Keep HEBREW explanation under 12 words. Format raw JSON: { "original": "...", "options": ["Correct option...", "Incorrect 1...", "Incorrect 2...", "Incorrect 3..."], "explanation": "Short Hebrew explanation" }`;
...
async function generateFullExam() {
    ...
    const promptSentences = `Expert Amirnet test writer. Generate JSON array of 12 "Sentence Completion" questions (Upper-B2/C1). RULES: 1. Academic sentences (15-25 words). 2. Blank '___' tests a challenging word. 3. CRITICAL: For each question, the 3 distractors MUST have completely different meanings from the correct answer. 4. CRITICAL: Force extreme topic diversity! Each of the 12 sentences MUST be about a completely different specific academic field (e.g., 1 about geology, 1 about art history, 1 about astrophysics, 1 about linguistics, etc.). 5. Format raw JSON array: [ { "sentence": "...", "options": ["CorrectWord", "Dist1", "Dist2", "Dist3"], "correctIndex": 0, "explanation": "Short Hebrew explanation" } ] IMPORTANT: Exactly 12 objects. First option (index 0) MUST be correct. Keep explanation under 10 words.`;

    const promptRestates = `Expert Amirnet test writer. Generate JSON array of 6 "Restatement" questions (Upper-B2/C1). RULES: 1. Academic original (18-25 words). CRITICAL: Force high variety in sentence structures! MAXIMUM ONE sentence starting with "Despite" or "Although". Mix Conditionals, Cause/Effect, etc. 2. CRITICAL: Force extreme topic diversity! Each of the 6 sentences MUST cover a distinct specific academic subject (e.g., marine biology, ancient Rome, macroeconomics). 3. Correct restatement MUST significantly alter syntactic structure. 4. All 4 options roughly same word count. 5. Format raw JSON array: [ { "original": "...", "options": ["Correct restatement.", "Dist 1.", "Dist 2.", "Dist 3."], "correctIndex": 0, "explanation": "Short Hebrew explanation" } ] IMPORTANT: Exactly 6 objects. First option (index 0) MUST be correct. Keep explanation under 12 words.`;

    const promptReading = `Expert Amirnet test writer. Generate JSON object for "Reading Comprehension". RULES: 1. Standard academic text (3 short paragraphs). CRITICAL: Pick a highly specific, random academic topic (e.g., the history of cartography, deep-sea hydrothermal vents, Byzantine architecture) to avoid repetitive AI themes. 2. Generate 5 questions. 3. All 4 options roughly same length per question. 4. Format raw JSON object: { "title": "Academic Title", "paragraphs": ["Par 1...", "Par 2...", "Par 3..."], "questions": [ { "question": "...", "options": ["Correct answer.", "Dist 1.", "Dist 2.", "Dist 3."], "correctIndex": 0, "explanation": "Short Hebrew explanation" } ] } IMPORTANT: First option (index 0) MUST be correct. Keep explanations under 12 words.`;
